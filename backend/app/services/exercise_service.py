...
from app.schemas.exercise import ExerciseCreate, ExerciseUpdate, ExerciseSearchQuery
...
class ExerciseService:
    ...
    def search(self, search_query: ExerciseSearchQuery, skip: int = 0, limit: int = 100) -> List[Exercise]:
        query = self.db.query(Exercise)
        ...
        if search_query.muscle_groups:
            query = query.filter(Exercise.muscle_groups.ilike(f"%{search_query.muscle_groups}%"))
        
        # Filter by difficulty level
        if search_query.difficulty_level:
            query = query.filter(Exercise.difficulty_level == search_query.difficulty_level)
        
        # Filter by equipment needed
        if search_query.equipment_needed:
            query = query.filter(Exercise.equipment_needed == search_query.equipment_needed)
    ...
    def get_by_category(self, category: str, skip: int = 0, limit: int = 100) -> List[Exercise]:
        return (
    ...
    def get_by_muscle_group(self, muscle_group: str, skip: int = 0, limit: int = 100) -> List[Exercise]:
        return (
            self.db.query(Exercise)
            .filter(and_(
    ...
    def get_by_equipment(self, equipment: str, skip: int = 0, limit: int = 100) -> List[Exercise]:
        return (
            self.db.query(Exercise)
            .filter(and_(Exercise.equipment_needed == equipment, Exercise.is_active == True))
            .order_by(Exercise.name)
            .offset(skip)
            .limit(limit)
            .all()
    ...
    def get_categories(self) -> List[str]:
        ...
        result = self.db.query(Exercise.category).filter(
            and_(Exercise.category.isnot(None), Exercise.is_active == True)
        ).distinct().all()
        return [row[0] for row in result if row[0]]

    def get_muscle_groups(self) -> List[str]:
        """Get list of unique muscle groups."""
        result = self.db.query(Exercise.muscle_groups).filter(
